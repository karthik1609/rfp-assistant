...
def _build_messages(preprocess_payload: Dict[str, Any]) -> list[Dict[str, str]]:
    summary = (
        preprocess_payload.get("key_requirements_summary") or "RFP preprocess summary"
    )
...
def _build_requirements_messages(
    ...
    snapshot = {
        ...
        "response_structure_requirements_count": (
...
def _build_build_query_messages(
    build_query_payload: Dict[str, Any],
) -> list[Dict[str, str]]:
    ...
    resp_summary = str(
        build_query_payload.get("response_structure_requirements_summary") or ""
    )
...
def store_preprocess_result(
    source_text: str, preprocess_payload: Dict[str, Any]
) -> bool:
    if not source_text:
    ...
    if ok:
        logger.info(
            "Mem0: stored preprocess record for user_id=%s at %s",
            user_hash[:12],
            STORE_PATH,
        )
    else:
        logger.warning(
            "Mem0: failed to store preprocess record for user_id=%s", user_hash[:12]
        )
    return ok


# function to store extracted requirements into local memory
def store_requirements_result(
    source_text: str, requirements_payload: Dict[str, Any]
) -> bool:
    if not source_text:
    ...
    try:
        ...
        logger.warning(
            "Failed to hash essential text for Mem0 requirements storage: %s", exc
        )
        return False
    ...
    if ok:
        logger.info(
            "Mem0: stored requirements record for user_id=%s at %s",
            user_hash[:12],
            STORE_PATH,
        )
    else:
        logger.warning(
            "Mem0: failed to store requirements record for user_id=%s", user_hash[:12]
        )
    return ok


# function to store build-query payload into local memory
def store_build_query_result(
    source_text: str, build_query_payload: Dict[str, Any]
) -> bool:
    if not source_text:
    ...
    try:
        ...
        logger.warning(
            "Failed to hash essential text for Mem0 build query storage: %s", exc
        )
        return False
    ...
    if ok:
        logger.info(
            "Mem0: stored build-query record for user_id=%s at %s",
            user_hash[:12],
            STORE_PATH,
        )
    else:
        logger.warning(
            "Mem0: failed to store build-query record for user_id=%s", user_hash[:12]
        )
    return ok
...
def store_edit_memory(source_text: str, edit_payload: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.warning(
            "Failed to hash source text for Mem0 edit memory storage: %s", exc
        )
        return False
    ...
    if ok:
        logger.info(
            "Mem0: stored edit memory record for user_id=%s with %d changed sentences at %s",
            user_hash[:12],
            len(edit_payload.get("changed_sentences") or []),
            STORE_PATH,
        )
    else:
        logger.warning(
            "Mem0: failed to store edit memory record for user_id=%s", user_hash[:12]
        )
    return ok
...
def search_memories(
    query: str, max_results: int = 5, stage: Optional[str] = None
) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    if not query or not STORE_PATH.exists():
        logger.debug(
            "Mem0 search skipped: empty query or missing store (exists=%s)",
            STORE_PATH.exists(),
        )
        return results
    ...
    logger.info(
        "Mem0 search starting: method=%s query_len=%d stage=%s max_results=%d",
        RETRIEVAL_METHOD,
        len(query),
        stage,
        max_results,
    ...
    if RETRIEVAL_METHOD == "embeddings" and results:
        try:
            ...
            logger.info(
                "Mem0: computed query embedding (len=%d)", len(q_emb) if q_emb else 0
            )
            if not q_emb:
            ...
            for rec in results:
                ...
                logger.debug(
                    "Mem0: candidate doc fingerprint present: %s, emb_len=%d",
                    rec.get("user_id") or "<no-user>",
                    len(doc_emb) if doc_emb else 0,
                )
                if not doc_emb:
                    continue
            ...
            logger.info(
                "Mem0 embeddings retrieval: %d candidates scored, returning top %d",
                len(scored),
                min(max_results, len(scored)),
            )
            return scored[:max_results]
        except Exception as e:
            logger.warning(
                "Embeddings retrieval failed, falling back to token matches: %s", e
            )

    results.sort(key=lambda r: r.get("score", 0), reverse=True)
    logger.info(
        "Mem0 token retrieval: %d matches found, returning top %d",
        len(results),
        min(max_results, len(results)),
    )
    for r in results[: min(5, len(results))]:
        try:
            logger.debug(
                "Mem0 match: user_id=%s score=%.4f snippet=%s",
                (r.get("user_id") or "<no-user>")[:12],
                r.get("score", 0.0),
...
def _get_embedding(text: str) -> List[float]:
    ...
    try:
        if os.environ.get("HF_TOKEN"):
            ...
            logger.debug(
                "Mem0 embedding: HF response length=%d", len(emb) if emb else 0
            )
            return list(emb)
        elif os.environ.get("AZURE_OPENAI_API_KEY"):
            client = get_azure_client()
            logger.debug(
                "Mem0 embedding: using Azure client, model=%s", EMBEDDING_MODEL
            )
            resp = client.embeddings.create(model=EMBEDDING_MODEL, input=text)
            emb = resp.data[0].embedding
            logger.debug(
                "Mem0 embedding: Azure response length=%d", len(emb) if emb else 0
            )
            return list(emb)
    except Exception as e:
        ...
        return []
    logger.debug(
        "No embedding client available for mem0 (HF_TOKEN / AZURE_OPENAI_API_KEY missing)"
    )
    return []
...
def _embedding_for_record_cached(record: Dict[str, Any], doc_text: str) -> List[float]:
    ...
    try:
        with EMBED_CACHE_PATH.open("a", encoding="utf-8") as f:
            f.write(
                json.dumps(
                    {"fp": fp, "embedding": emb, "ts": int(time.time())},
                    ensure_ascii=False,
                )
                + "\n"
            )
