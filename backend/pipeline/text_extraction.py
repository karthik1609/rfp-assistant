...
def _docx_to_images(docx_path: Path) -> List[Image.Image]:
    ...
    with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp_pdf:
        ...
        try:
            ...
            logger.info(
                "[Vision OCR Fallback] Step 2: Converting PDF to images (DPI: 200)..."
            )
            images = convert_from_path(str(tmp_pdf_path), dpi=200)
            logger.info(
                "[Vision OCR Fallback] Converted DOCX to %d page image(s)", len(images)
            )
            return images
        finally:
            try:
...
def _extract_text_from_pdf_direct(pdf_path: Path) -> str:
    try:
        ...
        logger.warning(
            "[PDF Direct Extraction] pdfplumber library not available, skipping direct extraction"
        )
        return ""
    try:
        logger.info(
            "[PDF Direct Extraction] Starting direct text extraction from PDF: %s",
            pdf_path.name,
        )
        text_parts: List[str] = []
        total_pages = 0
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            for page_num, page in enumerate(pdf.pages, 1):
                ...
                if page_text:
                    ...
                    logger.info(
                        "[PDF Direct Extraction] Page %d/%d: extracted %d characters",
                        page_num,
                        total_pages,
                        len(page_text),
                    )
                    ...
                    logger.warning(
                        "[PDF Direct Extraction] Page %d/%d: no text found (may be image-only)",
                        page_num,
                        total_pages,
                    )
        full_text = "\n\n--- Page Break ---\n\n".join(text_parts)
        ...
        logger.info(
            "[PDF Direct Extraction] Completed: %d characters from %d/%d pages with text",
            len(full_text),
            pages_with_text,
            total_pages,
        )
        ...
        logger.error(
            "[PDF Direct Extraction] Failed with error: %s", str(e), exc_info=True
        )
        return ""
...
def _extract_text_from_docx_direct(docx_path: Path) -> str:
    try:
        ...
        logger.warning(
            "[DOCX Direct Extraction] python-docx library not available, skipping direct extraction"
        )
        return ""
    try:
        logger.info(
            "[DOCX Direct Extraction] Starting direct text extraction from DOCX: %s",
            docx_path.name,
        )
        ...
        for para in doc.paragraphs:
            if para.text.strip():
                ...
                para_count += 1
        logger.info(
            "[DOCX Direct Extraction] Found %d paragraphs with text", para_count
        )
        ...
        for table_idx, table in enumerate(doc.tables, 1):
            ...
            if table_text_parts:
                ...
                table_count += 1
                logger.info(
                    "[DOCX Direct Extraction] Table %d: extracted %d rows",
                    table_idx,
                    len(table_text_parts),
                )
        full_text = "\n\n".join(text_parts)
        logger.info(
            "[DOCX Direct Extraction] Completed: %d characters from %d paragraphs and %d tables",
            len(full_text),
            para_count,
            table_count,
        )
        ...
        logger.error(
            "[DOCX Direct Extraction] Failed with error: %s", str(e), exc_info=True
        )
        return ""
...
def extract_text_from_file(path: PathLike) -> str:
    ...
    if suffix == ".pdf":
        logger.info(
            "[Text Extraction] Attempting direct PDF extraction (pdfplumber)..."
        )
        ...
        if suffix == ".docx":
            logger.info(
                "[Text Extraction] Attempting direct DOCX extraction (python-docx)..."
            )
            direct_text = _extract_text_from_docx_direct(p)
            extraction_method = "python-docx"
        else:
            logger.info(
                "[Text Extraction] .doc files not supported for direct extraction, skipping to OCR"
            )
        ...
        try:
            ...
            logger.error(
                "[Text Extraction] Failed to read text file: %s", e, exc_info=True
            )
            direct_text = ""
            extraction_method = "plain-text"
    else:
    ...
    if direct_text:
        ...
        logger.warning(
            "[Text Extraction] Document may be image-based or scanned. Falling back to vision model OCR..."
        )
    else:
        logger.warning(
            "[Text Extraction] Direct extraction failed or returned no text."
        )
        logger.warning(
            "[Text Extraction] Falling back to vision model OCR (this will take longer)..."
        )
    logger.info("=" * 80)
    ...
    if not images:
        logger.error(
            "[Text Extraction] ✗ ERROR: No images generated from file: %s", path
        )
        logger.warning(
            "[Text Extraction] Returning any text from direct extraction (if available)"
        )
    ...
    logger.info(
        "[Text Extraction] Method: Vision model OCR (%s)", TEXT_EXTRACTION_MODEL
    )
    logger.info(
        "[Text Extraction] Extracted: %d characters from %d page(s)",
        len(ocr_text),
        len(images),
    )
    logger.info("=" * 80)
