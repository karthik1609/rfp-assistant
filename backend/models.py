...
class ExtractionResult(BaseModel):
    translated_text: str = Field(
        default="", description="Translated text (empty for extraction agent)"
    )
    language: str = Field(description="ISO language code (e.g., 'en', 'fr')")
    key_requirements_summary: str = Field(
        default="", description="Summary of key requirements as markdown bullets"
    )
    raw_structured: Dict[str, Any] = Field(
...
class RequirementItem(BaseModel):
    ...
    category: str = Field(
        description="Category tag (e.g., 'Architecture', 'Security', 'SLA')"
    )
...
class StructureDetectionResult(BaseModel):
    ...
    structure_description: str = Field(
        default="", description="Description of the required structure"
    )
...
class BuildQuery(BaseModel):
    ...
    solution_requirements_summary: str = Field(
        description="Summary of solution requirements"
    )
    response_structure_requirements_summary: str = Field(
        description="Summary of response structure requirements"
    )
    extraction_data: Dict[str, Any] = Field(
        description="Extraction agent data (codes, language, etc.)"
    )
    confirmed: bool = Field(
        default=False, description="Whether the build query has been confirmed by human"
    )
...
class Question(BaseModel):
    ...
    requirement_id: Optional[str] = Field(
        default=None,
        description="ID of the requirement this question relates to (None for build query questions)",
    )
    question_text: str = Field(description="The actual question")
    context: str = Field(description="Why this question is important")
    category: str = Field(
        description="Category: technical, business, implementation, etc."
    )
    priority: str = Field(description="Priority: high, medium, or low")
    asked_at: Optional[str] = Field(
        default=None, description="Timestamp when question was asked"
    )
    answered: bool = Field(
        default=False, description="Whether question has been answered"
    )
...
class Answer(BaseModel):
    ...
    question_text: Optional[str] = Field(
        default=None, description="The question text that was answered"
    )
    answer_text: str = Field(description="The answer provided by the user")
    answered_at: Optional[str] = Field(
        default=None, description="Timestamp when answer was provided"
    )
...
class ConversationContext(BaseModel):
    ...
    requirement_id: Optional[str] = Field(
        default=None, description="Current requirement being processed"
    )
    ...
    def get_qa_context(self) -> str:
        ...
        for answer in self.answers:
            question = next(
                (q for q in self.questions if q.question_id == answer.question_id), None
            )
            if question:
