...
def _clarity_check(requirement_text: str, structure_text: Optional[str] = None) -> dict:
    if not requirement_text:
        return {
            "clarity": "unclear",
            "questions": ["Requirement text is empty"],
            "raw": "",
        }
    ...
    if any(
        k in low
        for k in ("unclear", "not clear", "need", "clarif", "missing", "ambigu")
    ):
...
def run_response_agent(
    ...
    if knowledge_base is not None:
        try:
            ...
            logger.info(
                "Included fusionAIx knowledge base context in prompt (%d chars)",
    ...
    if structure_detection:
        is_implicit_structure = not structure_detection.get(
        ...
        has_explicit_sections = any(
            keyword in struct_lower for keyword in explicit_keywords
        )

        is_implicit_structure = (
            not struct_summary
            or struct_summary == "No response structure requirements found."
            or not has_explicit_sections
        )
        logger.info(
    ...
    try:
        ...
        logger.info(
            "Clarity check result: %s (questions=%d)",
            clarity.get("clarity"),
        ...
        if clarity.get("clarity") == "unclear":
            try:
                retrieved_memories = search_memories(
                    req_summary or "", max_results=3, stage="requirements"
                )
                if retrieved_memories:
                    ...
                    logger.info(
                        "Included %d local memory snippets in prompt due to clarity check (ids/scores=%s)",
            ...
            logger.debug(
                "Skipping local memory retrieval; requirement considered clear by LLM"
            )
    except Exception as e:
        logger.warning(
            "Clarity check failed, falling back to retrieving local memories: %s", e
        )
        try:
            retrieved_memories = search_memories(
                req_summary or "", max_results=3, stage="requirements"
            ...
            if retrieved_memories:
                logger.info(
                    "Included %d local memory snippets in prompt (fallback)",
                    len(retrieved_memories),
                )
        except Exception as mem_exc:
        ...
        if search_query:
            ...
            if retrieved_edit_memories:
                ...
                for m in retrieved_edit_memories:
                    ...
                    ids_scores.append(f"{uid}:{score:.3f}")
                logger.info(
                    "Included %d edit memory snippets in prompt (ids/scores=%s)",
    ...
    if max_tokens is None:
        ...
        if is_implicit_structure:
            max_tokens = min(
                200, 32769 - estimated_input_tokens - 1000
            )  # ~500 chars for 5 sentences
        else:
            max_tokens = min(
                2500, 32769 - estimated_input_tokens - 1000
            )  # ~10000 chars
        logger.info(
