...
def _run_requirements_agent_cached(essential_text: str) -> RequirementsResult:
    text_input = (
        essential_text[:30000] if len(essential_text) > 30000 else essential_text
    )
    ...
    logger.info(
        "Requirements prompt tokens: system=%d, user=%d, total=%d",
        system_tokens,
        user_tokens,
        total_input_tokens,
    )
    ...
    logger.debug(
        "Requirements agent: Raw LLM response (first 500 chars): %s", content[:500]
    )
    logger.debug(
        "Requirements agent: Raw LLM response (last 500 chars): %s",
    ...
    def _parse_json_safely(raw: str) -> dict:
        ...
        try:
            ...
            logger.debug(
                "Requirements agent: Successfully parsed JSON. Top-level keys: %s",
            ...
            logger.error(
                "Requirements agent: failed to parse JSON at position %s: %s",
                error_pos,
                str(e),
            )
            logger.error(
                "Requirements agent: Problematic JSON (first 2000 chars): %s",
                cleaned[:2000],
            )
            logger.error(
                "Requirements agent: Problematic JSON (last 500 chars): %s",
            ...
            try:
                            ...
                            if cleaned[i] == "[":
                                ...
                                if bracket_count == 0:
                                    ...
                                    try:
                                        ...
                                        logger.warning(
                                            "Requirements agent: Failed to parse solution_requirements array: %s",
                                            parse_err,
                                        )
                                        partial["solution_requirements"] = []
                                    break
                            ...
                            if cleaned[i] == "[":
                                ...
                                if bracket_count == 0:
                                    ...
                                    try:
                                        ...
                                        logger.warning(
                                            "Requirements agent: Failed to parse response_structure_requirements array: %s",
                                            parse_err,
                                        )
                ...
                if partial:
                    logger.warning(
                        "Requirements agent: Partially parsed JSON, using extracted fields"
                    )
                    logger.warning(
                        "Requirements agent: solution_requirements: %d items, response_structure_requirements: %d items",
                ...
                logger.error(
                    "Requirements agent: Partial extraction also failed: %s", parse_exc
                )
            logger.error(
                "Requirements agent: Raw LLM response (first 3000 chars): %s",
                raw[:3000],
            )
            raise ValueError(
                f"LLM requirements agent returned invalid JSON: {str(e)}"
            ) from e

    try:
        data = _parse_json_safely(content)
        logger.debug(
            "Requirements agent: Parsed JSON successfully. Keys: %s", list(data.keys())
        )
        logger.debug(
            "Requirements agent: solution_requirements count: %d, response_structure_requirements count: %d",
        ...
        logger.error(
            "Requirements agent: Raw response (first 2000 chars): %s", content[:2000]
        )
        raise
    solution_reqs = []
    solution_raw = data.get("solution_requirements", []) or []
    logger.info(
        "Requirements agent: Processing %d solution requirements", len(solution_raw)
    )
    for idx, req_dict in enumerate(solution_raw):
        try:
            ...
            logger.warning(
                "Failed to parse solution requirement #%d: %s, error: %s",
                idx,
                req_dict,
                e,
            )
            logger.debug(
                "Failed requirement dict keys: %s",
                list(req_dict.keys()) if isinstance(req_dict, dict) else "not a dict",
            )
    response_reqs = []
    response_raw = data.get("response_structure_requirements", []) or []
    logger.info(
        "Requirements agent: Processing %d response structure requirements",
        len(response_raw),
    )
    for idx, req_dict in enumerate(response_raw):
        try:
            ...
            logger.warning(
                "Failed to parse response structure requirement #%d: %s, error: %s",
                idx,
                req_dict,
                e,
            )
